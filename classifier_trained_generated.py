...
def main():
    ...
    prop_fake = [60000, 54000, 48000, 30000, 0]
    accuracy_all = []
    ...
    for fine_tune in range(2):
        if fine_tune == 0:
            ...
            for run in range(len(prop_real)):
                ...
                data_real = Subset(train_data_real,indices[:prop_real[run]])
                data_fake = Subset(fake_data,indices[:prop_fake[run]])
    ...
    fig = plt.figure()
    ax2 = fig.gca()
    plt.plot([0, 10, 20, 50, 100], accuracy_all_fine, label='Fine-tuning')
    plt.plot([0, 10, 20, 50, 100], accuracy_all_shuffle, label='Shuffled')
    plt.legend(loc=4)
